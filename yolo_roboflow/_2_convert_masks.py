...
def collect_pairs():
    ...
    for stem_dir in sorted(SAM3_ROOT.iterdir()):
        ...
        if not mask_dir.exists() or not frame_dir.exists():
            continue
...
def main():
    ...
    if not pairs:
        print("Nothing to convert — run _0_gen_data.py first.")
        return
    ...
    (YOLO_ROOT / "bamboo.yaml").write_text(yaml)
    ...
    print("Next: python _3_upload_to_roboflow.py")
